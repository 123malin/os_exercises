...
> 代码见ucore_lab/labcodes_answer/lab8_result
> 从下面可以看出文件访问的大致执行过程，首先在输入每一个字母的时候，都会调用sys_read() -> sfsfile_read() -> file_read() -> vfs_inode read
> 然后打开文件的大致执行轨迹为sys_open() -> sysfile_open() -> file_open() -> vfs_open() -> vfs_inode open -> sfs_openfile() -> sysfile_open()打开文件
> 然后是读文件，大致流程为文件系统 -> vfs -> sfs -> disk0这样的一个读的顺序。
