...
>  初始化中断控制器，包括8259的相关设置；
>  初始化IDT，设置各中断向量的描述符，为之后发生中断能够由此找到服务例程的入口地址；
>  初始化时钟中断，包括设置时钟每秒的中断次数，以及通过中断控制器使能时钟中断； 使能整个系统的中断机制；
...
>  cprintf函数用到的三个外设是LPT（并口），CGA，串口，因为cprintf函数调用了cons_putc函数，而该函数的定义如下
...
> 先实现一个可以显示字符串的bootloader，然后在bootloader中实现从实模式切换到保护模式，并且建立堆栈来执行C代码。然后切换到C执行，读取内核镜像elf文件，切换到内代码执行。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 执行strace -c ./foo，在上述列表中可以查看到某个特定的syscall的次数。
