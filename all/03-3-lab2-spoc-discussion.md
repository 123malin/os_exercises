...
>pmm_init()首先调用了gdt_init对于GDT描述符表进行初始化。其中包括：
>>1.	对于GDT描述表的初始化。
>>2.	对于各个段寄存器的初始化。
>在实模式下获取物理内存分布，建立ARD表示格局。（这些信息存储在0x8000处）
>随后调用page_init()完成整个机器的物理内存管理
>建立页的目录表并对页表进行初始化，完成虚存到物理内存的映射
>置CR0的第31位为1，以使能页机制。
>更新GDT，使得段机制能够实现对等映射。
...
>根据qemu最后提示setup timer interrupts，这里产生了一个中断，但是我们还没有对IDT初始化，因此产生异常qemu无法处理这个异常，程序退出。
...
> 执行asm volatile ("int $10")语句之后，系统调用10会产生一个trap，系统进入kernel_debug模式，执行结果如下：
...
```
    ...
    int main()
    {
        ...
        scanf("%x%x", &va,&pa);
        unsigned int tmp1, tmp2, tmp3, tmpa;
        tmp1 = (va & 0xffc00000) >> 22 ;
        tmp2 = (va & 0x003ff000) >> 12 ;
        tmp3 = (va & 0x00000fff); 
        tmpa = (pa & 0xfffff000);
        tmpa |= 0x003;    
        pde_idx = tmp1;
