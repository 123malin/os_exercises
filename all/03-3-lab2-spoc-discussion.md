...
```
 ...
 ```
```
答：为了建立页机制，首先要初始化GDT并建立段机制；探测系统物理内存布局，管理空闲物理内存：建立一一映射关系的二级页表：使能分页机制：使能页表结构主要通过enable_paging函数实现，通过Icr3指令把页目录表的起始地址存入CR3寄存器中，通过Icr0指令把cr0的CR0_PG标志位设置为1，执行完毕后，ucore就进入了分页模式。
